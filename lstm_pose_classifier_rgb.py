...
def rgb_setting_bar():
    setting = 'RGB_settings'
    cv2.namedWindow(setting)
    cv2.moveWindow(setting, 1280, 0)

    cv2.createTrackbar('R_MAX', 'RGB_settings', 0, 255, onChange)
    cv2.setTrackbarPos('R_MAX', 'RGB_settings', config.upperColor_RGB[0])
    cv2.createTrackbar('R_MIN', 'RGB_settings', 0, 255, onChange)
    cv2.setTrackbarPos('R_MIN', 'RGB_settings', config.lowerColor_RGB[0])
    cv2.createTrackbar('G_MAX', 'RGB_settings', 0, 255, onChange)
    cv2.setTrackbarPos('G_MAX', 'RGB_settings', config.upperColor_RGB[1])
    cv2.createTrackbar('G_MIN', 'RGB_settings', 0, 255, onChange)
    cv2.setTrackbarPos('G_MIN', 'RGB_settings', config.lowerColor_RGB[1])
    cv2.createTrackbar('B_MAX', 'RGB_settings', 0, 255, onChange)
    cv2.setTrackbarPos('B_MAX', 'RGB_settings', config.upperColor_RGB[2])
    cv2.createTrackbar('B_MIN', 'RGB_settings', 0, 255, onChange)
    cv2.setTrackbarPos('B_MIN', 'RGB_settings', config.lowerColor_RGB[2])
...
def main():
    ...
    while True:
        ...
        if ret:
            ...
            R_MAX = cv2.getTrackbarPos('R_MAX', 'RGB_settings')
            R_MIN = cv2.getTrackbarPos('R_MIN', 'RGB_settings')
            G_MAX = cv2.getTrackbarPos('G_MAX', 'RGB_settings')
            G_MIN = cv2.getTrackbarPos('G_MIN', 'RGB_settings')
            B_MAX = cv2.getTrackbarPos('B_MAX', 'RGB_settings')
            B_MIN = cv2.getTrackbarPos('B_MIN', 'RGB_settings')
            lower = np.array([R_MIN, G_MIN, B_MIN])
            higher = np.array([R_MAX, G_MAX, B_MAX])
            ...
            # 하늘 제거용 마스크 생성
            sky_mask = cv2.inRange(rgb_frame, (105, 0, 195), (120, 40, 255))
            sky_mask = cv2.dilate(sky_mask, None, iterations=1)
            sky_mask = cv2.bitwise_not(sky_mask)
            
            # 땅 제거용 마스크 생성
            land_mask = cv2.inRange(rgb_frame, (10, 0, 150), (30, 80, 255))
            ...
            rgb_frame_find = cv2.cvtColor(frame_bg_rm, cv2.COLOR_BGR2RGB)
            ...
            color_mask_find = cv2.inRange(rgb_frame_find, lower, higher)
            color_mask_find = cv2.erode(color_mask_find, None, iterations=1)
            color_mask_find = cv2.dilate(color_mask_find, None, iterations=3)
            
            color_mask_see = cv2.inRange(rgb_frame, lower, higher)
            rgb_picker = cv2.bitwise_and(frame, rgb_frame, mask=color_mask_see)
            ...
            rgb_picker = cv2.resize(rgb_picker, dsize=(640, 360), interpolation=cv2.INTER_AREA)
            cv2.imshow(winname4, rgb_picker)
